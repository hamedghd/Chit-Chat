    ...
    try {
      if (status === "granted") {
        ...
        if (!result.cancelled) {
          ...
          this.props.onSend({ image: imageUrl, text: 'image:' });
        }
      }
    } catch (error) {
  ...
  takePhoto = async () => {
    ...
    try {
      if (status === "granted") {
        ...
        if (!result.cancelled) {
          const imageUrl = await this.uploadImageFetch(result.uri);
          this.props.onSend({ image: imageUrl, text: "my taken photo:" });
        }
      }
    } catch (error) {
      console.log(error.message);
      ...
      if (status === "granted") {
        ...
        if (result) {
          this.props.onSend({
            ...
            text: `my location: latitude ${result.coords.latitude} , longitude ${result.coords.longitude}`,
          });
        }
      }
    } catch (error) {
